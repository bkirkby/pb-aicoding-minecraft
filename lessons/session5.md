...
First, the mod stops depending on you. Right now the pet only thinks if someone started llama-server by hand. Today the mod checks the machine, fetches llama.cpp and the model itself, tells the player what it's doing, starts the server, and shuts it down when the game closes, with the canned lines waiting at every step where something's missing.
...
- If class wifi is poor, the download path gets demonstrated up to the progress bar and then cancelled. That's fine. The point is the check and the progress, not the bytes.
...
- Before downloading, check whether the machine can even run it: enough
  free disk for the download, enough memory to load the model. If it
  can't, don't download.
- Tell the player what's happening: that a download is starting, how big
  it is, and progress as it goes. The player should never wonder if the
  game is stuck.
- If a download fails, don't retry forever. Clean up and try again next
...
4. Download both files, with a message that says so and progress as it goes. If it fails, clean up, canned lines for now, try again next launch.
5. Then go to step 2.
...
- **When does it start?** At world join, so the model is ready by the time the pet wakes up? Or only once there's a sentient pet, so players who never get that far never download anything? Waking the pet is the emotional moment from session 3, and a download message landing on top of it would step on it. Your call, but think about it.
...
list. Include the folder, the numbers, and what we decided about when the
download starts.
```
...
## Step 4 — Checking, and downloading

```
Now build the check-and-download part: if the files aren't there, check
disk and memory. If the machine can handle it, tell the player a download
is starting and how big it is, download both files with progress shown in
the game, then start the server. If the machine can't handle it, say so
once and use canned lines. Build it.
```

To test it, make the files disappear without deleting them. Rename the model file, adding `.bak` to the end. Then:

1. Launch, join a world. The mod should notice the model is missing and tell you it's downloading. Read that message as a player would. Is it clear what's happening, how big it is, and that the game is still playable meanwhile?
2. Watch the progress. Walk around while it runs. The game should not stutter. If it does, that's a bug report: the download is on the game thread.
3. If the wifi is good, let it finish. The pet should start thinking without you doing anything else. If it isn't, quit partway. The mod should clean up the partial file, and the next launch should start over rather than choke on it.
4. Rename the model back and confirm the mod finds it and skips the download.

Then the other branch. Ask the AI to lower the memory limit in the capability check far above what the machine has, rebuild, and launch. The mod should tell you once that the machine can't run the model, and the pet should use canned lines with no download attempted. Put the limit back.

There's a conversation to have here about what a mod owes the player when it does something big on their machine. Your instructor has more on that.
...
Then find the jar in `build/libs`. That file is the mod. Anyone with Minecraft, Fabric, and the same version can drop it in their `mods` folder and play what you made. The first time they play, it'll fetch the model onto their machine and tell them what it's doing, because you built it that way.
...
- A mod that sets all of that up for a stranger, and keeps working when it can't
...
- Trying a bigger model, like Qwen2.5 7B, on a machine with a real GPU, and seeing how the pet changes
- Teaching the mod to detect a GPU and pick the model and llama.cpp build to match, falling back to the CPU build when the GPU one won't start. There's a full spec for this in the [GPU Detection and Model Selection](../guides/gpu-model-selection.md) guide
...
**It downloads again every time you join.** The check for "already on disk" isn't finding the files, or the download is landing somewhere other than where the check looks. Bug report.
...
**The disk fills up.** Two and a half gigabytes is two and a half gigabytes. The check in the flow should have caught it. If it didn't, that's the bug.
